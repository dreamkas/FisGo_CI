...
curl --request POST \
"${RELAY_SERVER}?chat_id=${CHAT}&parse_mode=markdown&text=Для продукта *${PRODUCT_TYPE}* выпущена новая версия - *${VERSION}*.%0A%0A${RELEASE_NOTES}%0A%0A[DirPatch](${DOWNLOAD_LINK})"
